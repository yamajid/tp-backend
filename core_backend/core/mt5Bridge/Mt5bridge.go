...
func (b *MT5Bridge) startWhileForListening(conn net.Conn, sessionID string) error {
	...
	for {
		...
		case RECONNECTING:
			log.Printf("[StartWhileForListening] EA %s is reconnecting", sessionID)
			// EA sent RECONNECTING message - acknowledge and continue
...
func (b *MT5Bridge) parseMessageType(message string) MessageType {
	if strings.Contains(message, "HELLO") {
		return HELLO
	} else if strings.Contains(message, "RECONNECTING") {
		return RECONNECTING
...
func (b *MT5Bridge) processSymbolMetadata(sessionID string, message string) {
	...
	// Check if GBPUSD variants exist
	gbpVariants := []string{"GBPUSD", "GBPUSDm", "GBPUSD.fx", "GBP/USD", "GPYUSD"}
	log.Printf("[processSymbolMetadata] Checking for GBP/USD variants...")
	for _, variant := range gbpVariants {
		if _, exists := b.symbolMetadata[variant]; exists {
			log.Printf("[processSymbolMetadata]   ✓ Found: %s", variant)
		}
	}
}
...
func (b *MT5Bridge) ResolveSymbol(canonical string) string {
	...
	log.Printf("[ResolveSymbol] ⚠️  No mapping found for canonical symbol: %s", canonical)
	log.Printf("[ResolveSymbol] Available symbols in metadata: %d", len(b.symbolMetadata))
	if len(b.symbolMetadata) > 0 {
		log.Printf("[ResolveSymbol] Sample symbols: ")
		count := 0
		for symbol := range b.symbolMetadata {
			if count < 10 {
				log.Printf("  - %s", symbol)
				count++
			}
